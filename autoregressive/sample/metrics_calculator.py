...
class VitExtractor:
    ...
    def __init__(self, model_name, device):
        ...
        # self.model = torch.hub.load('../pretrained_models', model_name, source='local').to(device)
        # # name_map = {
        ...
        # # }
        # name_map = {
        #     'dino_vits8' : 'dino-vits8',
        #     'dino_vits16': 'dino-vits16',
        #     'dino_vitb8' : 'dino-vitb8',
        #     'dino_vitb16': 'dino-vitb16',
        # }

        # # print(model_name,1111111111)
        ...
        # # 从本地路径加载（如果存在），否则自动下载
        # local_model_path = f"/mnt/xmap_nas_ml/jdy/code/AutoRegressive/pretrained_models/{model_name}"
        ...
        # else:
        #     # 自动下载（建议提前下载好）
        #     self.model = ViTModel.from_pretrained(model_name).to(device).eval()

        dino_repo_path = 'pretrained_models/dino'
        
        # 安全检查
        if not os.path.exists(os.path.join(dino_repo_path, 'hubconf.py')):
            raise RuntimeError(
                f"hubconf.py not found in {dino_repo_path}. "
                "Please run: git clone https://github.com/facebookresearch/dino.git /mnt/xmap_nas_ml/jdy/code/dino"
            )
...
class MetricsCalculator:
    def __init__(self, device) -> None:
        self.device=device
        self.clip_metric_calculator = CLIPScore(model_name_or_path="../pretrained_models/clip-vit-large-patch14").to(device)
