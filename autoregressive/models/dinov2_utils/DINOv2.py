...
class DinoVisionTransformer(nn.Module):
    ...
    def get_feature_distill(self, x, idx, masks=None):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i == idx:
                # print(i,1111)
                break
        # x_norm = partial(nn.LayerNorm, eps=1e-6)(x[0].shape[-1], elementwise_affine=False)(torch.concat(x, dim=-1))
        x_norm =  self.norm(x)
        return x_norm[:, 1:]
    

    def get_feature_distill_loss(self, x, idx, masks=None):
        x = self.prepare_tokens_with_masks(x, masks)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i == idx:
                # print(i,1111)
                break
        x_norm = partial(nn.LayerNorm, eps=1e-6)(x[0].shape[-1], elementwise_affine=False)(x)
        return x_norm[:, 1:]
    
    def get_feature_distill_loss_wonorm(self, x, idx, masks=None):
        x = self.prepare_tokens_with_masks(x, masks)
    ...
    def forward_features(self, x, masks=None):
        ...
        return {
            ...
            "x_norm_clstoken":self.norm(x)[:, :1],
        }
    
    def forward_features_loss_list(self, x, masks=None):
        if isinstance(x, list):
            return self.forward_features_list(x, masks)
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            x_mid = partial(nn.LayerNorm, eps=1e-6)(x[0].shape[-1], elementwise_affine=False)(x)
            x_list.append(x_mid[:, 1:])
        x_list.append(self.norm(x)[:, 1:])
        return x_list
    
    def forward_features_loss_list_cls(self, x, masks=None):
        if isinstance(x, list):
            return self.forward_features_list(x, masks)
